...
def get_parser(add_help=True):
    ...
    parser.add_argument(
        "--dataset",
        choices=["ImageNet", "CIFAR10", "CXR8", "ISIC2018", "ISIC2018CINet", "APTOS", "VinDrCXR", "Herlev"],
        help="The dataset."
    )
    parser.add_argument(
        "--base_network", 
        help="The model config or base network to use."
    )
    parser.add_argument(
        "--experiment_name",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Pretrained model to load weights from.",
    )
    parser.add_argument(
        "--features_only",
        action="store_true",
        help="Do not load classifier weights, just features.",
    )
