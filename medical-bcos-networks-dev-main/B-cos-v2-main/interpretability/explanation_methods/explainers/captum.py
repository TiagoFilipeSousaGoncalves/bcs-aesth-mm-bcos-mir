...
from bcos.models.densenet import BaselineDenseNet
from bcos.models.cinet import CINet
...
class GradCam(CaptumDerivative):
    ...
    def attribute(self, img, target, **kwargs):
        ...
        if isinstance(self.model, BaselineDenseNet) or (isinstance(self.model, CINet) and self.model.is_bcos == False):
            if isinstance(self.model, BaselineDenseNet): var_features = F.relu(var_features, inplace=False)
            out = F.adaptive_avg_pool2d(var_features, (1, 1))
