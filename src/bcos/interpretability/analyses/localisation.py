...
from localisation_configs import configs
from utils import Analyser, get_explainer_factory
...
class LocalisationAnalyser(Analyser):
    ...
    def compute_sorted_confs(self, force_confidence_calculation: bool = False) -> None:
        ...
        assert datamodule.NUM_CLASSES >= self.config["n_imgs"], "The n_imgs must be less than the number of classes."
        ...
        with torch.inference_mode():
            for img, tgt in tqdm(loader, desc="Calculating confidences"):
                ...
                out = out.sigmoid()
                print(out)
                logits, classes = out.max(1)
                print(logits, classes)

                # Adjust the shape of the labels according to the database
                if dataset == 'CXR8':
                    print(tgt)
                    tgt = tgt[:, classes] # tgt to explain is the predicted class with the highest prob
                else:
    ...
    def analysis(self) -> Dict[str, Any]:
        ...
        print(f"There are {len(fixed_indices)} valid images (i.e. with conf > {self.config['conf_thresh']})")
        print(f"Your chosen sample_size is {sample_size} and the grid_size is {n_imgs**0.5}. You can choose a bigger sample_size as long as it is lower than {len(fixed_indices) / n_imgs}.")
        assert sample_size * n_imgs <= len(fixed_indices), f"There are only {len(fixed_indices)} valid images (i.e. with conf > {self.config['conf_thresh']}), and the config has sample_size of {sample_size}.\nPlease choose a sample size lower than {len(fixed_indices) / n_imgs}."
        metric = []
        ...
        for count in tqdm(range(sample_size), desc="Analysis..."):
            multi_img, tgts, offset = self.make_multi_image(
                n_imgs, dataset, self.experiment.dataset, offset=offset, fixed_indices=fixed_indices
            )
        ...
        print()
        return {"localisation_metric": result, "localisation_metric_avg": np.array([np.mean(result)])}

    @staticmethod
    def make_multi_image(n_imgs, dataset, dataset_name, offset=0, fixed_indices=None):
        """
