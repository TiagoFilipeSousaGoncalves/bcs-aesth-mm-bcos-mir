...
# Project Imports
from .data_utilities import CXR8Dataset, ISIC2018Dataset, ISIC2018CINetDataset, APTOSDataset, VinDrCXRDataset, HerlevDataset
...
__all__ = ["ImageNetDataModule", "CIFAR10DataModule", "ClassificationDataModule", "CXR8DataModule", "ISIC2018DataModule", "ISIC2018CINetDataModule", "APTOSDataModule", "VinDrCXRDataModule", "HerlevDataModule"]
...
class ClassificationDataModule(pl.LightningDataModule):
    ...
    def __init__(self, config):
        ...
        # FIXME: In some cases we need the key "data", in others we don't. 
        # FIXME: Correct this.
        try:
            self.batch_size = config["batch_size"]
            self.num_workers = config["num_workers"]
            print("W/out 'data' key.")
        except:
    ...
    def test_dataloader(self):
        return data.DataLoader(
            self.eval_dataset if self.test_dataset is None else self.test_dataset,
            self.batch_size,
            shuffle=False,
            num_workers=self.num_workers,
            pin_memory=True,
...
class ImageNetDataModule(ClassificationDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        if "data" in self.config:
            transform = self.config["data"]["test_transform"]
        else:
...
# Class: CXR8DataModule
class CXR8DataModule(ClassificationDataModule):
    NUM_CLASSES: int = 15

    def setup(self, stage: str, return_id: bool = False) -> None:
        DATA_ROOT = settings.DATA_ROOT
        if stage == "fit":
            self.train_dataset = CXR8Dataset(
                images_dir=os.path.join(DATA_ROOT, "images-resized"),
                metadata_dir=os.path.join(DATA_ROOT, "metadata"),
                subset="train",
                transform=self.config["train_transform"],
                tr_perc=self.config["tr_perc"],
            )


        # FIXME: W/ or W/out key 'data
        try:
            transform=self.config["data"]["test_transform"]
            print("W/ 'data' key.")
        except:
...
# Class: VinDrCXRDataModule
class VinDrCXRDataModule(ClassificationDataModule):
    NUM_CLASSES: int = 23
    ...
    def setup(self, stage: str, return_id: bool = False) -> None:
        ...
        if stage == "fit":
            self.train_dataset = VinDrCXRDataset(
                images_dir=os.path.join(DATA_ROOT, "images-resized"),
                metadata_dir=os.path.join(DATA_ROOT, "annotations-resized"),
                subset="train",
                transform=self.config["train_transform"],
                tr_perc=self.config["tr_perc"],
            )
        ...
        self.eval_dataset = VinDrCXRDataset(
            images_dir=os.path.join(DATA_ROOT, "images-resized"),
            metadata_dir=os.path.join(DATA_ROOT, "annotations-resized"),
            subset="validation",
            transform=transform,
        ...
        self.test_dataset = VinDrCXRDataset(
            images_dir=os.path.join(DATA_ROOT, "images-resized"),
            metadata_dir=os.path.join(DATA_ROOT, "annotations-resized"),
            subset="test",
            transform=transform,
...
# Class: ISIC2018CINetDataModule
class ISIC2018CINetDataModule(ClassificationDataModule):
    NUM_CLASSES: int = 7

    def setup(self, stage: str, return_id: bool = False) -> None:
        DATA_ROOT = settings.DATA_ROOT
        if stage == "fit":
            self.train_dataset = ISIC2018CINetDataset(
                images_dir=os.path.join(DATA_ROOT, "images-resized"),
                metadata_dir=os.path.join(DATA_ROOT, "metadata"),
                task=3,
                subset="train",
        ...
        self.eval_dataset = ISIC2018CINetDataset(
            images_dir=os.path.join(DATA_ROOT, "images-resized"),
            metadata_dir=os.path.join(DATA_ROOT, "metadata"),
            task=3,
            subset="validation",
        ...
        self.test_dataset = ISIC2018CINetDataset(
            images_dir=os.path.join(DATA_ROOT, "images-resized"),
            metadata_dir=os.path.join(DATA_ROOT, "metadata"),
            task=3,
            subset="test",
...
# Class: ISIC2018DataModule
class ISIC2018DataModule(ClassificationDataModule):
    NUM_CLASSES: int = 7
    ...
    def setup(self, stage: str, return_id: bool = False) -> None:
        ...
        self.eval_dataset = ISIC2018Dataset(
            images_dir=os.path.join(DATA_ROOT, "images-resized"),
            metadata_dir=os.path.join(DATA_ROOT, "metadata"),
            task=3,
            subset="validation",
...
class APTOSDataModule(ClassificationDataModule):
    NUM_CLASSES: int = 5
    ...
    def setup(self, stage: str, return_id: bool = False) -> None:
        ...
        self.eval_dataset = APTOSDataset(
            images_dir=os.path.join(DATA_ROOT, "images-resized"),
            metadata_dir=os.path.join(DATA_ROOT, "metadata"),
            subset="validation",
            transform=transform,
...
# Class: HerlevDataModule
class HerlevDataModule(ClassificationDataModule):
    NUM_CLASSES: int = 7

    def setup(self, stage: str, return_id: bool = False) -> None:
        DATA_ROOT = settings.DATA_ROOT
        if stage == "fit":
            self.train_dataset = HerlevDataset(
        ...
        self.eval_dataset = HerlevDataset(
            pickle_file=os.path.join(DATA_ROOT, "data.pickle"),
            subset="validation",
            transform=transform,
            return_id=return_id
        ...
        self.test_dataset = HerlevDataset(
            pickle_file=os.path.join(DATA_ROOT, "data.pickle"),
            subset="test",
            transform=transform,
            return_id=return_id
