...
def load_model_state_dict_from_training_ckpt(
    ...
    """
    num_suitable_loaders = sum(loader.is_suitable_state_dict(training_ckpt) for loader in LOADERS)
    if num_suitable_loaders > 1:
        ...
        print(("Unsupported checkpoint format! Loading training checkpoint manually..."))
        try:
            model_state_dict = torch.load(training_ckpt, map_location="cpu")
        except:
...
def get_state_dict_and_training_ckpt_from_save_dir(
    ...
    # Load checkpoint from weights path
    elif reload == ReloadTypes.WEIGHTS:
        training_ckpt = device_safe_load_state_dict_from_path(weights)
    

    # Load the best or best (any) checkpoints
    else:
    ...
    # Verbose: print extra information if needed
    if verbose:
...
def _print_information_of_loaded_state_dict(
    ...
    try:
        ...
        val_acc = None
        metric_key = "eval_acc" if not ema else "eval_acc_ema"
